<x-admin-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Usuarios</h1>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Gestiona los usuarios del sistema</p>
            </div>
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 hover:bg-primary-700 dark:hover:bg-primary-500 text-white text-sm font-medium rounded-lg transition-colors">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @foreach($user->roles as $role)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            {{ $role->name === 'admin' ? 'bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-400' : '' }}
                                            {{ $role->name === 'author' ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-800 dark:text-blue-400' : '' }}
                                            {{ $role->name === 'user' ? 'bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-400' : '' }}
                                        ">
                                            {{ ucfirst($role->name) }}
                                        </span>
                                    @endforeach
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.users.edit', $user) }}" class="text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400" title="Editar">
                                        ...
                                        @else
                                            <span class="text-gray-400 dark:text-gray-600" title="No puedes eliminarte a ti mismo">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
